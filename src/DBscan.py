...
def DBscanAlgorithmLoop(
    min_epsilon, max_epsilon, epsilon_step, min_min_samples, max_min_samples, folder
):
    # Wczytanie danych
    ...
    while epsilon <= max_epsilon + epsilon_step:
        for min_samples in range(min_min_samples, max_min_samples + 1):
            ...
            if p_cluster > 0:
                distances = pdist(
                    dataForDBSCAN[labels == 0]
                )  # Oblicz odległości tylko dla punktów w klastrze
...
def DBscanChart(folder, show_picture, p_id):
    ...
    sc = plt.scatter(
        ...
        s=5,
        alpha=0.6,
        norm=PowerNorm(gamma=50, vmin=y_per),
    )
    plt.title(f"dbScan p={p_id}")
    plt.xlabel("Epsilon")
    plt.ylabel("Min Samples")
    plt.colorbar(
        sc, label="Średnia wartość", format="%.2f"
    )  # Dodajemy formatowanie do kolorów
    plt.grid(True)
    ...
    plt.scatter(
        epsilon_max_test,
        min_samples_max_test,
        color="red",
        marker="x",
        label="Maksymalna wartość testu",
    )
    plt.close()
    plt.clf()
